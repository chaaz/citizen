...
This fork is used to create the chaaz-repo version of citizen:0.6.0.
Mostly, it overrides the ngrok download process, since the ngrok website
certificate is incorrect, making the `postinstall` for the npm ngrok
package impossible to run.

[source, sh]
....
docker build -t us-west1-docker.pkg.dev/dragon-291307/chaaz/citizen:0.6.0 .
docker push us-west1-docker.pkg.dev/dragon-291307/chaaz/citizen:0.6.0
....
